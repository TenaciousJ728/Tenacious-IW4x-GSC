...
init()
{
    ...
    if (getDvarInt("rando_debug_loadout") == 1)
        iPrintLnBold("^5Rando loaded | Interval: ^2" + getDvarInt("rando_interval") + "s | Attachments: ^2" + getDvarInt("rando_attachments_max"));

    if (getDvarInt("rando_attachments_max") > 2)
        setDvar("rando_attachments_max", 2);
    if (getDvarInt("rando_attachments_max") < -1)
        setDvar("rando_attachments_max", -1);
    ...
    // ====================== WEAPON BLACKLIST ======================
    // Edit this array to completely remove any weapons from random selection.
    // (Useful for riotshield_mp, deserteaglegold_mp, or any other base you don't want.)
    level.weaponBlacklist = [];
    level.weaponBlacklist[level.weaponBlacklist.size] = "onemanarmy_mp";
    // Add more lines here as needed, e.g.:
    // level.weaponBlacklist[level.weaponBlacklist.size] = "yourweapon_mp";
    ...
    if (getDvarInt("rando_attachments_max") >= 0)
...
getRandomLoadout()
{
    ...
    amount = getDvarInt("rando_mains_amount");
    ...
    for (i = 0; i < amount && temp.size > 0; i++)
    {
        ...
        baseAttach = getDvarInt("rando_attachments_max");
        if (baseAttach == -1) baseAttach = 0;
        if (baseAttach > 2)   baseAttach = 2;
